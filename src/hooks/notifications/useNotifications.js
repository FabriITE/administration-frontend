...
import { getNotifications, markNotificationAsRead, markNotificationAsUnread, readAllNotifications } from '../../../utils/api';
...
export const useNotifications = () => {
    ...
    const readNotification = async (notification_id) => {
        const x = await markNotificationAsRead({notification_id : notification_id})
        await getNotificationsAll(employee_id)
    }
    
    const unReadNotification = async (notification_id) => {
        const x = await markNotificationAsUnread({notification_id : notification_id})
        await getNotificationsAll(employee_id)
    }
